...
Below is our initial main regression model, with all variables and factors we intend to include (here interaction terms are not included). Result shows significant F test result, which means linear relationship exists. However, R-squared is small and some variables are not significant. We may need a partial F test.

```{r}
mod1 <- lm(ADM_RATE ~ CONTROL_1+CONTROL_2+COSTT4_A+AVGFACSAL+FEMALE+PCT_BA+PCT_WHITE+PCT_ASIAN+PCT_BLACK+POVERTY_RATE+CONTROL_1*FEMALE+CONTROL_1*PCT_ASIAN+CONTROL_1*POVERTY_RATE+CONTROL_1*COSTT4_A+CONTROL_1*AVGFACSAL+CONTROL_1*PCT_WHITE+CONTROL_1*PCT_BA, data=data)
summary(mod1)
```

#Partial F test

First we examine whether the interaction terms between CONTROL_1 (which is whether the university is public) and other variables are still needed. We found that 

We reject the null hypothesis that coefficientS of CONTROL_1, CONTROL_1*FEMALE, CONTROL_1*ASIAN are zero. So we included them in our model.

```{r}
mod1 <- lm(ADM_RATE ~ CONTROL_1+CONTROL_2+COSTT4_A+AVGFACSAL+FEMALE+PCT_BA+PCT_WHITE+PCT_ASIAN+PCT_BLACK+POVERTY_RATE, data=data)
mod2 <- lm(ADM_RATE ~ CONTROL_1+CONTROL_2+COSTT4_A+AVGFACSAL+FEMALE+PCT_BA+PCT_WHITE+PCT_ASIAN+PCT_BLACK+POVERTY_RATE+CONTROL_1*FEMALE+CONTROL_1*PCT_ASIAN+CONTROL_1*POVERTY_RATE+CONTROL_1*COSTT4_A+CONTROL_1*AVGFACSAL+CONTROL_1*PCT_WHITE+CONTROL_1*PCT_BA, data=data)
anova(mod1,mod2)
```

We fail to reject the null hypothesis that coefficient of CONTROL_1*PCT_WHITE, CONTROL_1*COSTT4_A, CONTROL_1*AVGFACSAL are zero, so we choose to delete these variables from the model.
...
plot(r ~ data$ADM_RATE, main="res against AdPages", xlab="AdPages", ylab="res")
plot(r ~ data$COSTT4_A, main="res against SubRevenue", xlab="SubRevenue", ylab="res")
plot(r ~ data$AVGFACSAL, main="res against NewsRevenue", xlab="NewsRevenue", ylab="res")
plot(r ~ data$PCT_WHITE, main="res against NewsRevenue", xlab="NewsRevenue", ylab="res")
plot(r ~ data$PCT_ASIAN, main="res against NewsRevenue", xlab="NewsRevenue", ylab="res")
plot(r ~ data$FEMALE, main="res against NewsRevenue", xlab="NewsRevenue", ylab="res")
plot(r ~ data$POVERTY_RATE, main="res against NewsRevenue", xlab="NewsRevenue", ylab="res")
qqnorm(r)
qqline(r)
```
...
```{r}
mod <- lm(ADM_RATE ~ CONTROL_1+COSTT4_A+AVGFACSAL+FEMALE+PCT_WHITE+log(POVERTY_RATE)+CONTROL_1*FEMALE+CONTROL_1*log(PCT_ASIAN), data=data)
summary(mod)
